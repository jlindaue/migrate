 ...
 * @param Wye Cost of lateral offset from line from start to end point
 * @param Wyaw Cost of angle from line from start to end point
 * @param Wc Cost of curvature
 * @param Wdc Cost of curvature change
 */
 ...
 * @param wpts Ordered vector of waypoints we want to plan through, in whatever coordinates
 *                  waypoint is defined by 4 numbers:
 *                  - x[m]
 *                  - y[m]
 *                  - heading[rad]
 *                  - curvature[1/m]
 ...
 * @param Yoff vector of y offsets
 * @param Ns vector of sampe numbers corresponding to Yoff
 * @param p Ssructure with algorithm parameters
...
void grad_G(VectorXd& X, std::vector<double> Yoff, std::vector<int> Ns, params& p, std::vector<T>& gg);
 ...
 * @param ang angle which we want to modify
 * @return modified angle
 */
double in_pi_range(double ang);
/**
 * @brief Transformation into the car frame (init_state = [0,0,0,curvature])
 * @param wpts  waypoints to transform
 * @param wpts_t reference to transformed waypoints
 */
